...
export default function PropertyFiltering({searchTerm}) {
  ...
  useEffect(() => {
    ...
    if (priceRange.length > 0) {
      const filtered = refItems.filter(
        (elm) => {
          // Extract the numeric part of the price by removing the currency sign
          let priceStr = elm.price;
          if (priceStr.includes("$")) {
            priceStr = priceStr.split("$")[1];
          } else if (priceStr.includes("₵")) {
            priceStr = priceStr.split("₵")[1];
          }
          const numericPrice = Number(priceStr.split(",").join(""));
